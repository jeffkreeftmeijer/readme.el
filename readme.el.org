...
To run the script, run Emacs in =--batch= mode, open the file to be converted to Markdown (=readme.el.org=, for example), load =readme.el= and call the function with the desired output filename (=README.md=):
