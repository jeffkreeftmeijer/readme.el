...
To automatically generate README files on GitHub, add a workflow that runs =readme.el= whenever changes are pushed.
There are two ways to do this, either by [[#nix-workflow][running a Nix command]], or [[#submodule-workflow][adding the submodule to your repository and running the script directly]].

** As a Nix command
:PROPERTIES:
:CUSTOM_ID: nix-workflow
:END:

The =readme.el= repository includes a =flake.nix=, which provides a script with emacs and all dependencies installed.
Run it with =nix run= directly from the repository:

#+begin_src sh
  nix run github:jeffkreeftmeijer/readme.el -- org-readme.org README.md
#+end_src

To install Nix and run generate README files automatically, use [[https://github.com/cachix/install-nix-action][install-nix-action]] and ~readme.el~ in a GitHub workflow:
...
** As a submodule
:PROPERTIES:
:CUSTOM_ID: submodule-workflow
:END:

To use =readme.el= as a submodule, check it out into your repository and call it from a workdlow.
